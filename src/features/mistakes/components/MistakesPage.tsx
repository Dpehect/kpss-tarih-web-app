...
export function MistakesPage() {
  ...
  return (
    <div className="space-y-6">
              ...
              <Card key={attempt.id}>
                <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                  <div className="min-w-0">
                    <p className="text-xs font-black uppercase tracking-[0.22em] text-[#9a3412]">
                      Yanlış {index + 1} / {topic.title}
                    </p>
                    <h2 className="mt-3 text-2xl font-black tracking-[-0.05em] text-[var(--navy-900)]">
                      {question.stem}
                    </h2>
                    <div className="mt-5 grid gap-3 md:grid-cols-2">
                      <div className="rounded-[1.25rem] border border-[#9a3412]/25 bg-[#fff0e9] p-4">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#9a3412]">Senin cevabın</p>
                        <p className="mt-2 font-semibold text-[var(--navy-900)]">{attempt.selectedChoiceId}) {selectedChoice?.text}</p>
                      </div>
                      <div className="rounded-[1.25rem] border border-[#047857]/25 bg-[#dff8ef] p-4">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#047857]">Doğru cevap</p>
                        <p className="mt-2 font-semibold text-[var(--navy-900)]">{question.correctChoiceId}) {correctChoice?.text}</p>
                      </div>
                    </div>
                    <p className="mt-5 rounded-[1.25rem] border border-[var(--border-soft)] bg-white/74 p-4 leading-7 text-[var(--text-secondary)]">
      ...
      <section className="rounded-[2rem] border border-[var(--border-soft)] bg-[rgba(255,248,234,.82)] p-5 text-[var(--navy-900)] shadow-[var(--shadow-xs)]">
        <div className="flex items-center gap-3">
          <span className="grid size-10 place-items-center rounded-2xl bg-[var(--burgundy-soft)] text-[#9a3412]">
            <XCircle size={18} />
          </span>
          <p className="font-black">Yanlış sıfırlama sadece yanlış kayıtlarını temizler; konu, flashcard, deneme ve not ilerlemen korunur.</p>
        </div>
      </section>
    </div>
  );
