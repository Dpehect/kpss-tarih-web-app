  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="rounded-xl parchment-surface p-5">
        <p className="text-xs font-bold uppercase tracking-wider text-[#f6c465]">Filtre</p>
      ...
      <div className="rounded-xl parchment-surface p-6">
        <div className="flex flex-col gap-3 border-b border-white/10 pb-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs uppercase tracking-wider text-[#f6c465]">
        ...
        <h2 className="mt-7 text-2xl font-semibold leading-tight tracking-tight">{current.stem}</h2>
        ...
        <div className="mt-7 grid gap-3">
          {current.choices.map((choice) => {
            ...
            return (
              ...
              >
                <span className="mr-3 font-semibold text-[#f6c465]">{choice.id}</span>
                {choice.text}
              </button>
            );
          })}
        ...
        {selected ? (
          <div className="mt-6 rounded-xl bg-[#120b07]/56 p-5">
            <p className="font-semibold text-[#f6c465]">Açıklama</p>
            ...
            <button onClick={nextQuestion} className="mt-5 rounded-full bg-[#f2c15f] px-5 py-3 font-semibold text-[#120b07]">
              Sonraki soru
            </button>
          </div>
        ) : null}
