...
export function PastQuestionsPage({ selectedYear }: { selectedYear?: number }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-black/[0.08] bg-[#fffaf0]/84 p-5 shadow-[0_20px_70px_rgba(18,24,38,0.08)]">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          {filtered.map((question) => {
            ...
            return (
              <article key={question.id} className="rounded-[2rem] border border-black/[0.08] bg-[#fffaf0]/88 p-6 shadow-[0_20px_70px_rgba(18,24,38,0.08)]">
                <div className="flex flex-col gap-3 border-b border-black/[0.08] pb-5 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.22em] text-[#2447d8]">
                      {question.year} · {question.topicTitle}
                    </p>
                    <h2 className="mt-3 text-2xl font-black tracking-[-0.05em] text-[#111827]">{question.stem}</h2>
                  </div>
                  <span className="w-fit rounded-full bg-[#edf1ff] px-3 py-1 text-xs font-black text-[#2447d8]">
                    {question.pattern}
                  </span>
                </div>
                ...
                <button
                  ...
                  className="mt-5 inline-flex items-center gap-2 rounded-full border border-[#2447d8]/18 bg-[#edf1ff] px-4 py-2 text-sm font-black text-[#2447d8]"
                >
                  <Lightbulb size={16} />
                ...
                {showHint ? (
                  <p className="mt-4 rounded-[1.25rem] border border-[#2447d8]/18 bg-[#edf1ff] p-4 text-sm font-semibold leading-7 text-[#2447d8]">
                    {question.hint}
                  </p>
                ) : null}
                ...
                {answered ? (
                  <div className={`mt-5 rounded-[1.5rem] border p-5 ${
                    isCorrect ? "border-[#2f8f75]/25 bg-[#dff8ef]" : "border-[#be684b]/25 bg-[#fff0e9]"
                  }`}>
                    <p className="font-black text-[#111827]">{isCorrect ? "Doğru" : "Yanlış"}</p>
        ...
        <aside className="space-y-4">
          <div className="dark-surface rounded-[2rem] p-6">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#f0bd59]">Telif notu</p>
            <h2 className="mt-3 text-3xl font-black tracking-[-0.06em]">Birebir soru değil, eğilim çalışması.</h2>
            <p className="mt-4 text-sm leading-7 text-[#fffaf0]/68">
              ÖSYM sorularının telif hakları ÖSYM'ye aittir. Bu bölüm resmi arşive yönlendirir ve özgünleştirilmiş pratik üretir.
            </p>
          </div>

          <div className="rounded-[2rem] border border-black/[0.08] bg-[#fffaf0]/88 p-5">
            <p className="kicker">Yıl özeti</p>
            <p className="mt-3 text-4xl font-black tracking-[-0.07em] text-[#111827]">{year}</p>
