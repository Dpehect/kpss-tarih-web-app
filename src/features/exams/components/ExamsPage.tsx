  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {exams.map((exam) => (
          <a key={exam.id} href={`/exams/${exam.id}`} className="rounded-[2rem] parchment-surface p-6 transition hover:-translate-y-1">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#f6c465]">{exam.durationMinutes} dk</p>
            <h2 className="mt-4 text-2xl font-black tracking-[-0.04em]">{exam.title}</h2>
