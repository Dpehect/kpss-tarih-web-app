  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
        <div className="bureau-stage relative overflow-hidden rounded-[2.35rem] p-5 md:p-8">
          ...
          <div className="relative z-10 mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-[-0.06em] text-[var(--bureau-inverse)]">Sürükle, seç, bağı oku.</h2>
            </div>
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[.08] px-4 py-2 text-sm font-black text-[var(--bureau-inverse)]">
            ...
            <div className="flex min-w-[980px] snap-x items-stretch gap-4">
              {enriched.map((event, index) => {
                ...
                return (
                  ...
                  >
                    <span className="grid size-10 place-items-center rounded-full bg-[var(--bureau-teal)] text-sm font-black text-[var(--bureau-inverse)]">{index + 1}</span>
                    <p className="mt-6 text-xs font-black uppercase tracking-[.22em] text-[var(--bureau-inverse-muted)]">{event.date}</p>
                    <h3 className="mt-3 text-xl font-black tracking-[-.04em] text-[var(--bureau-inverse)]">{event.title}</h3>
                    <p className="mt-3 text-sm leading-6 text-[var(--bureau-inverse-copy)]">{event.topicTitle}</p>
                  </motion.button>
                );
              })}
            </div>
        ...
        {selected ? (
          <motion.aside
            ...
            className="bureau-card rounded-[2rem] p-6"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-black uppercase tracking-[.22em] text-[var(--bureau-teal)]">{selected.date}</p>
                <p className="text-sm font-semibold text-[var(--bureau-muted)]">Seçili kayıt</p>
              </div>
            </div>

            <h2 className="mt-7 text-4xl font-black tracking-[-.07em] text-[var(--bureau-ink)]">{selected.title}</h2>
            <p className="mt-5 leading-8 text-[var(--bureau-copy)]">{selected.description}</p>

            <div className="mt-7 rounded-[1.35rem] border border-[var(--bureau-line)] bg-[rgba(255,250,242,.76)] p-4">
              <div className="flex items-center gap-2 text-sm font-black text-[var(--bureau-ink)]">
                <MapPinned size={17} />
                İlişkili konu
              </div>
              <p className="mt-2 text-sm font-semibold leading-6 text-[var(--bureau-copy)]">{topics.find((topic) => topic.id === selected.topicId)?.title ?? "Genel Tarih"}</p>
            </div>
          </motion.aside>
        ) : null}
      </section>
